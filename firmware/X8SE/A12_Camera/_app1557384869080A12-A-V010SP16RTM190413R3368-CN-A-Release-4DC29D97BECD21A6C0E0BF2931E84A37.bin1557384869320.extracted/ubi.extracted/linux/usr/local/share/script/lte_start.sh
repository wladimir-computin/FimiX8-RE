...
dns_queries ()
{
	...
	if [ "${dhcp_range}" != "" ]; then
		killall dnsmasq; sleep 0.1
		LOGGER "dnsmasq -5 -K --log-queries --user=root ${dhcp_range}"
		dnsmasq -5 -K --log-queries --user=root ${dhcp_range}
	fi
}
...
lte_stop ()
{
	...
	if [ "${vidpid}" != "" ]; then
		...
		if [ "${net}" != "" ]; then
			...
			#altair
			if [ "${Vendor}" == "216f" ] && [ -e /dev/ttyACM0 ]; then
...
auto_setting()
{
	...
	if [ "${APN}" == "" ]; then
		...
		if [ "${MCC}" == "" ] || [ "${MNC}" == "" ]; then
			...
			while [ "${MCC}" == "" ] && [ $n -ne 5 ]; do
				LOGGER "[trial $n]"
				echo -e "ATE0\r" | microcom -s 115200 -t 1000 ${UART_NODE}
				echo -e "AT+COPS=3,2\r" | microcom -s 115200 -t 1000 ${UART_NODE}
...
if [ -e /sys/bus/usb/devices/1-1/power/control ] && [ "${Vendor}" != "216f" ]; then
	echo auto > /sys/bus/usb/devices/1-1/power/control
else
	LOGGER "no power saving"
fi
