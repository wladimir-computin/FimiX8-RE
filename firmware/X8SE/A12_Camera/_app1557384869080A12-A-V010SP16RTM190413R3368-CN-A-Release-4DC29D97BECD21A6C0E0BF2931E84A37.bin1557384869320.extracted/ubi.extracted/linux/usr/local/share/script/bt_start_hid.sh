...
cleanup()
{
	...
	if [ "`ps -o args|grep /usr/libexec/bluetooth/bluetoothd|grep \"\-a\"|grep -v grep`" != "" ]; then
		/usr/local/share/script/bt_le_connect_hook.sh leadv5
		if [ "${blehid}" != "" ]; then
	...
	elif [ "`cat /pref/bt.conf |grep GATT_PERIPHERAL=yes`" != "" ]; then
		if [ "${btgatt_restart}" != "" ]; then
			DEVICE_NAME=`cat /pref/bt.conf | grep DEVICE_NAME | cut -c 13-`
			CMD_LOGGER btgatt-server -n ${DEVICE_NAME}
		fi
		which btgatt-server && /usr/local/share/script/bt_le_connect_hook.sh leadv5
	fi
}
...
start_hid ()
{
	...
	BTCTL_FIFO_LOGGER "quit"
	sleep 0.1
	bluetoothctl -d -p 0000 -a NoInputNoOutput
	...
	if [ "${tmp}" == "" ]; then
		echo -e "\e[1;35m cannot pair ${BT_MAC}, do it interactively \e[0m"
		cleanup
		exit 1
	fi
...
start_blehid ()
{
	...
	if [ "${cache_cmd}" == "no" ]; then
		...
		echo "${0} ${BT_MAC}" > /tmp/hid_gatt_disconnect_cb.sh
		echo "${hid_gatt_cmd}" > /tmp/hid_gatt_cmd.sh
		chmod 777 /tmp/hid_gatt_disconnect_cb.sh /tmp/hid_gatt_cmd.sh
	fi
	...
	if [ "${QUICK_BLE_MAC}" == "" ]; then
		...
		echo -e "\e[1;35m  1. echo \"${0} ${BT_MAC}\" > /tmp/hid_gatt_disconnect_cb.sh \e[0m"
		echo -e "\e[1;35m  2. echo \"${hid_gatt_cmd}\" > /tmp/hid_gatt_cmd.sh \e[0m"
		echo -e "\e[1;35m  3. chmod 777 /tmp/hid_gatt_disconnect_cb.sh /tmp/hid_gatt_cmd.sh \e[0m"
		echo -e "\e[1;35m  4. turn on BLE remote and run /tmp/hid_gatt_disconnect_cb.sh \e[0m"
		echo -e "\e[1;35m\n To skip auto-reconnect: \e[0m"
		echo -e "\e[1;35m  1. rm -f /tmp/hid_gatt_disconnect_cb.sh \n\e[0m"
	fi
}
...
hcitool_lescan ()
{
	if [ "`pidof hcitool`" != "" ]; then
		echo -e "\e[1;35m err: hcitool already started \e[0m"
		exit 1
	fi
	echo -e "\e[1;35m\n To skip auto-reconnect: \e[0m"
	echo -e "\e[1;35m\n   rm -f /tmp/hid_gatt_disconnect_cb.sh \n\e[0m"
	CMD_LOGGER hcitool lewlclr
	CMD_LOGGER hcitool lewladd ${QUICK_BLE_MAC}

	#hcitool will return when MAC found or btgatt-server connected
	echo -e "\e[1;35m hcitool lescan --passive --whitelist --amba \e[0m"
	result=`hcitool lescan --passive --whitelist --amba 2>&1`
	echo -e "\e[1;35m ${result} \e[0m"
	BT_MAC=`echo "${result}" | grep "${QUICK_BLE_MAC}" | head -n 1 | awk '{print $1}'`

	#btgatt-server connected
	if [ "${BT_MAC}" != "${QUICK_BLE_MAC}" ]; then
		#Central mode cannot run when peripheral mode is connected
		exit 1
	fi
...
##GO main flow
rm -f /tmp/hid_gatt_disconnect_cb.sh /tmp/hid_gatt_cmd.sh
#ia: interactive
if [ $# -gt 1 ]; then
	ia=1
fi
...
CMD_LOGGER killall btgatt-server
btgatt_restart=1
BT_NAME=${1}
...
while [ "${tmp}" == "" ] && [ $n -ne 15 ]; do
	...
	sleep 1
	devices=`echo "devices" | bluetoothctl 2>/dev/null | grep "^Device"`
	tmp=`echo "${devices}" | grep "${BT_NAME}" | head -n 1`
	echo "${devices}"
	echo -e "\e[1;35m\n ${tmp} \n\e[0m"
	interactive
done
...
if [ "${tmp}" == "" ]; then
	echo -e "\e[1;35m cannot find ${BT_NAME} \e[0m"
	cleanup
	exit 1
fi
