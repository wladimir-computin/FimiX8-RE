...
if [ "${1}" != "" ] && [ -e /tmp/wpa_supplicant.conf ]; then
	...
	wpa_supplicant -D${driver} -iwlan0 -c/tmp/wpa_supplicant.conf -s -B
...
WPA_SCAN ()
{
	if [ -e /sys/module/bcmdhd ]; then
		...
		wpa_supplicant -D${driver} -iwlan0 -C /var/run/wpa_supplicant -s -B -c /tmp/wpa_scan.conf
	else
		wpa_supplicant -D${driver} -iwlan0 -C /var/run/wpa_supplicant -s -B
	fi
	wpa_cli -i wlan0 scan
	echo "start scan for ${ESSID}"
	sleep 1
	scan_result=`wpa_cli -i wlan0 scan_r`
